...
fn rasterize_triangle(triangle: [Vec3; 3], collision_map: &DashMap<IVec3, u8>) {
    let min_x = triangle
        .iter()
    ...
    for x in min_x..=max_x {
        for y in min_y..=max_y {
            for z in min_z..=max_z {
                collision_map.insert(IVec3::new(x, y, z), 0);
            }
        }
    }
}
...
impl Game {
    ...
    pub fn insert_static_model_entity(
        ...
        let cam = unsafe { CAMERA.as_ref().unwrap() };
        let mut modent =
            ModelEntity::new_with_id(id, model_index, pos, scale, rot, csys, &cam, hostile);
        ...
        for mut model in self.non_static_model_entities.iter_mut() {
            ...
            if model.hostile {
                if model.attacktimer < model.attackinterval {
                    ...
                    let campos: Vec3 = unsafe { PLAYERPOS.snapshot().pos.into() };
    ...
    pub fn draw_models(&self) {
        ...
        unsafe {
            ...
            gl::Uniform1f(
                gl::GetUniformLocation(self.modelshader.shader_id, b"time\0".as_ptr() as *const i8),
