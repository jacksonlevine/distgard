...
impl CertInteractionEvent {
    ...
    pub fn apply_cert_verifier_action(
        ...
        if let Some(sender) = sender.take() {
            ...
            Err(QuintetError::CertificateActionAlreadyApplied)
        }
    }
}
...
///
/// ```
/// use jeffy_quintet::client::certificate::TrustOnFirstUseConfig;
/// let config = TrustOnFirstUseConfig {
///     known_hosts: jeffy_quintet::client::certificate::KnownHosts::HostsFile(
...
fn parse_known_host_line(
    ...
    let adr_str = parts.next().ok_or(QuintetError::InvalidHostFile)?;
    let serv_name = ServerName(RustlsServerName::try_from(adr_str)?);

    let fingerprint_b64 = parts.next().ok_or(QuintetError::InvalidHostFile)?;
    ...
    match fingerprint_bytes.try_into() {
        ...
        Err(_) => Err(Box::new(QuintetError::InvalidHostFile)),
    }
}
