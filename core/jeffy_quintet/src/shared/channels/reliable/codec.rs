...
pub struct QuintetProtocolCodecError {
    _priv: (),
}

impl fmt::Debug for QuintetProtocolCodecError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("QuintetProtocolCodecError").finish()
    }
}

impl fmt::Display for QuintetProtocolCodecError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("frame size too big")
    }
}

impl std::error::Error for QuintetProtocolCodecError {}

#[derive(Debug)]
pub struct QuintetProtocolCodecEncoder {
    // Maximum frame length
    max_frame_len: usize,
    raw_channel_id: u8,
}

impl QuintetProtocolCodecEncoder {
...
impl Encoder<Bytes> for QuintetProtocolCodecEncoder {
    type Error = io::Error;
    ...
    fn encode(&mut self, frame: Bytes, dst: &mut BytesMut) -> Result<(), io::Error> {
        if frame.len() > self.max_frame_len {
            return Err(io::Error::new(
                ...
                QuintetProtocolCodecError { _priv: () },
            ));
        }
...
#[derive(Debug)]
pub struct QuintetProtocolCodecDecoder {
    // Read state
    state: DecodeState,
    // Maximum frame length
    max_frame_len: usize,
...
impl QuintetProtocolCodecDecoder {
...
impl Decoder for QuintetProtocolCodecDecoder {
    type Item = BytesMut;
    type Error = io::Error;
