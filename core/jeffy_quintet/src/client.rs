...
pub const DEFAULT_KNOWN_HOSTS_FILE: &str = "Quintet/known_hosts";

/// Possible errors occuring while a client is connecting to a server
#[derive(thiserror::Error, Debug)]
pub enum QuintetConnectionError {
...
pub(crate) enum ClientAsyncMessage {
    ...
    ConnectionFailed(QuintetConnectionError),
...
/// Main Quintet client. Can open multiple [`Connection`] with multiple Quintet servers
///
/// Created by the [`QuintetClientPlugin`] or inserted manually via a call to [`bevy::prelude::World::insert_resource`]. When created, it will look for an existing [`AsyncRuntime`] resource and use it or create one itself.
#[derive(Resource)]
pub struct QuintetClient {
...
impl FromWorld for QuintetClient {
    fn from_world(world: &mut World) -> Self {
        ...
        QuintetClient::new(runtime.handle().clone())
    }
}
...
impl QuintetClient {
    ...
    ) -> Result<ConnectionLocalId, QuintetError> {
    ...
    pub fn close_connection(
        ...
        match self.connections.remove(&connection_id) {
            ...
            None => Err(QuintetError::UnknownConnection(connection_id)),
        }
    }

    /// Calls close_connection on all the open connections.
    pub fn close_all_connections(&mut self) -> Result<(), QuintetError> {
...
///
/// It is possbile to add both this plugin and the [`crate::server::QuintetServerPlugin`]
pub struct QuintetClientPlugin {
...
impl Default for QuintetClientPlugin {
    fn default() -> Self {
        Self {
...
impl Plugin for QuintetClientPlugin {
    fn build(&self, app: &mut App) {
        ...
        if !self.initialize_later {
            app.init_resource::<QuintetClient>();
        }
        ...
        app.add_systems(
            ...
            update_sync_client
                ...
                .run_if(resource_exists::<QuintetClient>),
        );
    }
}
...
pub fn client_connecting(client: Option<Res<QuintetClient>>) -> bool {
    match client {
...
pub fn client_connected(client: Option<Res<QuintetClient>>) -> bool {
    match client {
...
pub fn client_just_connected(
    ...
    client: Option<Res<QuintetClient>>,
) -> bool {
