...
use jeffy_quintet::{
    client::{
        ...
        QuintetClient, QuintetClientPlugin, DEFAULT_KNOWN_HOSTS_FILE,
    },
    server::{
        certificate::CertificateRetrievalMode, QuintetServer, QuintetServerPlugin,
...
fn trust_on_first_use() {
    ...
    // We received the client action request and asked to abort the connection
    {
        ...
        let client = client_app.world().resource::<QuintetClient>();
        assert!(
