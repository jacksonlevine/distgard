...
use jeffy_quintet::{
    client::{
        ...
        QuintetClient, QuintetClientPlugin,
    },
    server::{
        self, certificate::CertificateRetrievalMode, QuintetServer, QuintetServerPlugin,
...
pub fn start_simple_connection(mut client: ResMut<QuintetClient>, port: Res<Port>) {
    client
        .open_connection(
...
pub fn start_listening(mut server: ResMut<QuintetServer>, port: Res<Port>) {
    server
        .start_endpoint(
...
pub fn get_default_client_channel(app: &App) -> ChannelId {
    let client = app.world().resource::<QuintetClient>();
    client
        .connection()
        .get_default_channel()
...
pub fn get_default_server_channel(app: &App) -> ChannelId {
    let server = app.world().resource::<QuintetServer>();
    server
        .endpoint()
        .get_default_channel()
        .expect("Expected some default channel")
...
pub fn wait_for_client_message(
    ...
    let mut server = server_app.world_mut().resource_mut::<QuintetServer>();
...
pub fn wait_for_server_message(client_app: &mut App) -> (ChannelId, SharedMessage) {
    let mut client = client_app.world_mut().resource_mut::<QuintetClient>();
...
pub fn send_and_test_client_message(
    ...
    *msg_counter += 1;
    ...
    let client = client_app.world_mut().resource_mut::<QuintetClient>();
    client
        .connection()
